<script>
	...
	import { LeafletMap, TileLayer, Tooltip, Popup, Marker, Circle } from "svelte-leafletjs?client";
	...
	<react:Card type={typeColor} width="clamp(300px, 100%, 1200px)" style={{ margin: "auto", marginTop: "128px" }}>
		<react:Card.Content>
			...
			<react:Text
				...
				<react:Link href="https://google.com/maps/search/{location}" icon
					>Lokasyon: {earthquake.location}</react:Link
				>
			</react:Text>
			<react:Text>
		...
		<react:Card.Footer>
			...
			<react:Text small
				>Saat {parseInt(earthquake.date.slice(11, 13)) + 3}{earthquake.date.slice(13, 19)}</react:Text
			>
		</react:Card.Footer>
	</react:Card>
{:else}
