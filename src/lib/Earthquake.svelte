...
>
	<react:Card.Content>
		...
		<react:Text
			...
			<react:Link href="https://google.com/maps/search/{location}" icon>Lokasyon: {location}</react:Link>
		</react:Text>
	</react:Card.Content>
	<react:Card.Footer>
		<react:Text small
			>{ago(`${date.slice(0, 11)}${parseInt(date.slice(11, 13)) + 3}${date.slice(13, 19)}`)}</react:Text
		>
		<react:Text small>{date.slice("T", 10)}</react:Text>
		<react:Text small>Saat {parseInt(date.slice(11, 13)) + 3}{date.slice(13, 19)}</react:Text>
	</react:Card.Footer>
</react:Card>
